...
export default function ChargingStationsSection() {
  ...
  const locationPinUrl = "YOUR_IMAGE_URL_HERE"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <h2 className="text-5xl md:text-6xl font-bold mb-6 text-white">
            Find Our Charging Stations
          </h2>
          <p className="text-xl text-white/90 max-w-2xl mx-auto mb-6">
            Discover our strategically located charging stations across Nepal, designed for your convenience
          </p>
          ...
          <div className="space-y-0 max-w-7xl mx-auto">
            {stations.map((station, index) => (
              <div key={station.id} className="relative">
                {/* Vertical Dotted Line Segment (before pin) - Skip for first item */}
                {index > 0 && (
                  <div className="hidden md:block absolute left-1/2 transform -translate-x-1/2 top-0 h-20 border-l-4 border-dotted border-white z-0"></div>
                )}

                {/* Card and Pin Container */}
                <div className={`relative ${index === 0 ? 'pt-0' : 'pt-20'}`}>
                  {/* Location Pin - Absolutely positioned at center on vertical line */}
                  <div className="hidden md:block absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20" style={{ marginTop: '30px' }}>
                    <motion.div
                      ...
                      className="relative"
                    >
                      <div className="absolute inset-0 bg-red-400 rounded-full blur-lg opacity-40"></div>
                      <LocationPin size={70} />
                    </motion.div>
                  </div>

                  <div className="flex items-center">
                    {/* Left Side - Card and Line stopping before center pin */}
                    {index % 2 === 0 && (
                      <>
                        ...
                        {/* Horizontal Dotted Line - stops before center */}
                        <motion.div
                          initial={{ scaleX: 0 }}
                          whileInView={{ scaleX: 1 }}
                          viewport={{ once: true }}
                          transition={{ delay: 0.2, duration: 0.8 }}
                          className="hidden md:block border-t-4 border-dotted border-white origin-left ml-8"
                          style={{ width: 'calc(50% - 350px)' }}
                        ></motion.div>
                      </>
                    )}

                    {/* Right Side - Line from center and Card */}
                    {index % 2 !== 0 && (
                      <>
                        {/* Horizontal Dotted Line - starts after center */}
                        <motion.div
                          initial={{ scaleX: 0 }}
                          whileInView={{ scaleX: 1 }}
                          viewport={{ once: true }}
                          transition={{ delay: 0.2, duration: 0.8 }}
                          className="hidden md:block border-t-4 border-dotted border-white origin-right mr-8"
                          style={{ width: 'calc(50% - 160px - 32px)', marginLeft: 'calc(50% + 32px)' }}
                        ></motion.div>
